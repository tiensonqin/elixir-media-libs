defmodule Rtmp.Protocol.RawMessageTest do
  ...
  test "Can unpack SetChunkSize message" do
    ...
    assert %RtmpDetailedMessage{content: %Rtmp.Protocol.Messages.SetChunkSize{size: 4000}} =
             message
  end
  ...
  test "Can unpack Acknowledgement message" do
    ...
    assert %RtmpDetailedMessage{
             content: %Rtmp.Protocol.Messages.Acknowledgement{sequence_number: 25}
           } = message
  end
  ...
  test "Can unpack Window Acknowlegement Size message" do
    ...
    assert %RtmpDetailedMessage{
             content: %Rtmp.Protocol.Messages.WindowAcknowledgementSize{size: 26}
           } = message
  end
  ...
  test "Can unpack Set Peer Bandwidth message" do
    ...
    assert %RtmpDetailedMessage{
             content: %Rtmp.Protocol.Messages.SetPeerBandwidth{window_size: 20, limit_type: :soft}
           } = message
  end

  test "Can unpack User Control Stream Begin" do
    expected = %RtmpDetailedMessage{
      content: %Rtmp.Protocol.Messages.UserControl{
        type: :stream_begin,
        stream_id: 521,
        buffer_length: nil,
        timestamp: nil
  ...
  test "Can unpack amf 0 encoded command message" do
    expected = %RtmpDetailedMessage{
      content: %Rtmp.Protocol.Messages.Amf0Command{
  ...
  test "Can unpack amf0 encoded data message" do
    ...
    expected = %RtmpDetailedMessage{
      content: %Rtmp.Protocol.Messages.Amf0Data{parameters: amf_objects}
    }
