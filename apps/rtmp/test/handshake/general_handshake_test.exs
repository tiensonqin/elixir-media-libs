defmodule Rtmp.Handshake.GeneralHandshakeTest do
  ...
  test "Can parse full old format handshake" do
    ...
    assert {handshake,
            %Rtmp.Handshake.ParseResult{
              current_state: :waiting_for_data,
              # their p2 should match my p1
              bytes_to_send: <<1::4*8, 0::4*8, 555::1528*8>>
    ...
    # final result
    assert {_,
            %Rtmp.Handshake.HandshakeResult{
  ...
  test "Two old handshake instances can complete handshake against each other" do
    ...
    # packets 0 and 1
    assert {handshake1,
            %Rtmp.Handshake.ParseResult{current_state: :waiting_for_data, bytes_to_send: bytes1_2}} =
             Rtmp.Handshake.process_bytes(handshake1, bytes2_1)

    assert {handshake2,
            %Rtmp.Handshake.ParseResult{current_state: :waiting_for_data, bytes_to_send: bytes2_2}} =
  ...
  test "Two digest handshake instances can complete handshake against each other" do
    ...
    assert {server,
            %Rtmp.Handshake.ParseResult{current_state: :waiting_for_data, bytes_to_send: s0_s1_s2}} =
